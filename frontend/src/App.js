...
function App() {
  ...
  const sendData = () => {
    if (newEventsList.length === 0) {
      console.log("新しいイベントがありません");
      return;
    }

    // newEventsList のデータを transactions 形式に変換
    const newTransactions = newEventsList.map(event => ({
      date: event.start,
    ...
    axios.post('http://localhost:8001/api/transactions', {
      transactions: newTransactions
        ...
        .then(response => {
          ...
          // 送信後、transactions を更新して既存データに加える
